      ...
      <div class="flex-1 relative bg-black/50 p-4 flex items-center justify-center overflow-hidden">
        ...
        >
          <div
            ...
            @mousemove="handleRemoteMouseEvent(user.id, 'move', $event)"
            @mousedown="handleRemoteMouseEvent(user.id, 'down', $event)"
            @mouseup="handleRemoteMouseEvent(user.id, 'up', $event)"
            @wheel.prevent="handleRemoteWheelEvent(user.id, $event)"
            @contextmenu.prevent
            :class="{ 'cursor-crosshair': controllingUserId === user.id }"
          >
            <!-- Screen Stream (Main) -->
            <VideoPlayer
              ...
              :sreenStream="true"
              :pipMode="false"
              :disableDbClick="controllingUserId === user.id"
            />
            <!-- Camera Stream (Main) -->
            <VideoPlayer
              v-else
...
const handleRemoteMouseEvent = (userId: string, action: string, e: MouseEvent) => {
  if (controllingUserId.value !== userId) return

  // 如果视图被切换（主视图不是屏幕共享），则禁用控制，避免坐标错乱
  if (swappedViews.value.has(userId)) return
  ...
  if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
    // 窗口空间优化：计算实际视频内容的区域 (模拟 object-fit: contain)
    ...
    if (contRatio > vidRatio) {
      // 左右留黑边 (Pillarbox)
      renderH = contH
      renderW = renderH * vidRatio
      offsetX = (contW - renderW) / 2
      offsetY = 0
    } else {
      // 上下留黑边 (Letterbox)
      renderW = contW
      renderH = renderW / vidRatio
      offsetX = 0
  ...
  if (action === 'down' || action === 'up') {
    ...
    // 点击事件立即发送
    sendControlEvent(userId, { type: 'mouse', action, x, y, button })
    return
  }
  ...
  if (action === 'move') {
    ...
    const now = Date.now()
    if (now - lastMouseUpdate.value > mouseUpdateInterval) {
      sendControlEvent(userId, { type: 'mouse', action, x, y })
      lastMouseUpdate.value = now
    } else {
      // 确保最后一次移动也能发送
      moveTimer = setTimeout(() => {
        sendControlEvent(userId, { type: 'mouse', action, x, y })
        lastMouseUpdate.value = Date.now()
      }, mouseUpdateInterval)
    }
  }
}

const handleRemoteWheelEvent = (userId: string, e: WheelEvent) => {
  if (controllingUserId.value !== userId) return
  if (swappedViews.value.has(userId)) return

  // 发送滚轮事件，x/y 为滚动增量
  sendControlEvent(userId, {
    type: 'mouse',
    action: 'scroll',
    x: e.deltaX,
    y: e.deltaY,
...
const handleRemoteKeyDown = (e: KeyboardEvent) => {
  if (controllingUserId.value) {
    // Esc to exit control
    ...
    sendControlEvent(controllingUserId.value, { type: 'keyboard', key: e.key, keyState: 'down' })
    // Prevent default actions for some keys to keep focus on control
...
const handleRemoteKeyUp = (e: KeyboardEvent) => {
  if (controllingUserId.value) {
    sendControlEvent(controllingUserId.value, { type: 'keyboard', key: e.key, keyState: 'up' })
    if (!['F5', 'F12'].includes(e.key)) {
      e.preventDefault()
    }
  }
...
const lockKeyboard = async () => {
  ...
  if (nav.keyboard && nav.keyboard.lock) {
    try {
      // 锁定系统快捷键 (Alt+Tab, Win键等)，使其发送给远端而不是在本地触发
      ...
      console.log('Keyboard locked for remote control')
    } catch (e) {
...
const toggleIntercept = async (enable: boolean, targetId?: string) => {
  // 双重保险：优先调用 Web API 锁定键盘。
  if (enable) lockKeyboard()
  else unlockKeyboard()
  ...
  try {
    ...
    if (enable && targetId) {
      await invoke('start_intercept')
      if (!unlistenHook) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        unlistenHook = await listen('control-event-local', (event: any) => {
          sendControlEvent(targetId, event.payload)
        })
      }
    } else {
      await invoke('stop_intercept')
      if (unlistenHook) {
        unlistenHook()
        unlistenHook = null
      }
    ...
    console.warn('Tauri hook setup failed, relying on Web Lock:', e)
  }
}

watch(controllingUserId, (newId) => {
  toggleIntercept(!!newId, newId || undefined)
})
...
const startKeepAlive = () => {
  try {
    ...
    // 使用极低频率和音量，人耳不可闻，但能保持 AudioContext 活跃
...
onMounted(() => {
  ...
  window.addEventListener('keyup', handleRemoteKeyUp)
