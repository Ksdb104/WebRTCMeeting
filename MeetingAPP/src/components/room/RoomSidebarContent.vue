<template>
  <div class="flex flex-col h-full">
    ...
    <div class="flex border-b border-white/10">
      ...
      >
        {{ t('sidebar.members') }} ({{ users.size + 1 }})
        <div
    ...
    <div class="flex-1 overflow-y-auto bg-slate-900/50">
      ...
      <div v-if="activeTab === 'users'" class="p-4 space-y-4">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
              <span class="text-sm font-medium">{{ localUser.name }} {{ t('sidebar.me') }}</span>
              <span
        ...
        <div
          ...
          :key="user.id"
          class="relative flex items-center justify-between p-3 bg-white/5 rounded-lg border transition-all duration-300"
          :class="[
            user.isSpeaking
              ? 'border-green-500/50 bg-green-500/10 shadow-[0_0_15px_-3px_rgba(34,197,94,0.2)]'
              : interpretingUsers?.has(user.id) || receivingInterpretation?.has(user.id)
        ...
        >
          <!-- 同传状态横幅 -->
          <div
            v-if="interpretingUsers?.has(user.id) || receivingInterpretation?.has(user.id)"
            class="absolute top-0 left-0 right-0 bg-amber-500/20 text-amber-300 text-[10px] font-medium text-center py-0.5 rounded-t-lg"
          >
            🌐 {{ t('interpretation.active') }}
          </div>

          <div
            class="flex items-center gap-3"
            :class="{
              'mt-3': interpretingUsers?.has(user.id) || receivingInterpretation?.has(user.id),
            }"
          >
            <div
              class="w-8 h-8 rounded-full bg-linear-to-br from-blue-500 to-purple-500 flex items-center justify-center text-xs font-bold transition-all duration-300"
            ...
            <div class="flex flex-col">
              ...
              <span v-else class="text-xs text-gray-500 flex items-center gap-1">
                ...
                ></div>
                {{ user.connected ? t('sidebar.connected') : t('sidebar.connecting') }}
              </span>
            </div>
          </div>
          <div
          ...
          >
            <!-- 同传按钮 -->
            <button
              @click="handleInterpretClick(user.id)"
              :disabled="!canToggleInterpretation(user)"
              class="p-1 rounded transition-colors"
              :class="{
                'text-amber-400 bg-amber-400/10':
                  interpretingUsers?.has(user.id) || receivingInterpretation?.has(user.id),
                'text-gray-500 hover:text-gray-300 hover:bg-white/10':
                  !isInterpretationActive(user.id) && canToggleInterpretation(user),
                'text-gray-600 cursor-not-allowed': !canToggleInterpretation(user),
              }"
              :title="
                isInterpretationActive(user.id)
                  ? t('interpretation.stop')
                  : canToggleInterpretation(user)
                    ? t('interpretation.start')
                    : t('interpretation.cannotStart')
              "
            >
            ...
            <button
              v-if="user.isDesktop"
              @click="emit('requestControl', user.id)"
              class="p-1 hover:bg-white/10 rounded transition-colors"
              :title="t('sidebar.remoteAssist')"
            >
              <MousePointer2 class="w-4 h-4 text-blue-400" />
            </button>
...
import { Mic, MicOff, Video, VideoOff, Send, MousePointer2, Languages } from 'lucide-vue-next'
...
const isInterpretationActive = (userId: string) =>
  Boolean(props.interpretingUsers?.has(userId) || props.receivingInterpretation?.has(userId))

const canToggleInterpretation = (user: User) =>
  isInterpretationActive(user.id) || props.localUser.micOpen || user.micOpen

// 同传按钮点击：已激活时需要二次确认
const handleInterpretClick = (userId: string) => {
  const user = props.users.get(userId)
  if (!user || !canToggleInterpretation(user)) return

  if (isInterpretationActive(userId)) {
    if (confirm(t('interpretation.confirmStop'))) {
      emit('toggleInterpretation', userId)
    }
  } else {
    emit('toggleInterpretation', userId)
  }
